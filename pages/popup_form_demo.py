...
# --- Streamlit UI ---
st.title("📅 Delivery Calendar with Pop-Up Details")
...
for _, row in df.iterrows():
    events.append({
        ...
        "metadata": {key: str(value) for key, value in row.items()} # Attach full row details as metadata
    })
...
# --- Pop-up Details ---
if selected_event:
    st.markdown("### 📌 Selected Delivery Details")
    event_metadata = selected_event.get("metadata", {})

    # Display event details in a pop-up style
    with st.expander(f"📦 Order {event_metadata.get('Limit Order', 'N/A')}", expanded=True):
